...
def parse(driver, n, date_interval, autosave=10, keyword = "All"):
    ...
    for count in range(1, int(n)+1):
        ...
        if count == n:
            ...
            df.to_csv(dirpath + str(count) + ".csv", sep=";", encoding="utf-8")
            break
...
for interval in dates:
    ...
    if not pgs == 0:
        ...
        df.to_csv(res_dir + "/Result.csv", sep=";", encoding="utf-8")
        df.to_excel(res_dir +"/Result.xlsx")
    else:
